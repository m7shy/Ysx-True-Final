...
import { getMailboxConnection, connectionForMailbox } from '../creds/mailboxStore.js';
import type { ProviderName } from './types.js';

export { ProviderName };

export function parseProvider(raw: unknown, fallback: ProviderName = 'gmail'): ProviderName {
 ...
 */
export async function getImapConfig(userId: string, provider: ProviderName) {
...
export async function getSmtpConfig(userId: string, provider: ProviderName) {
  const conn = await getMailboxConnection(userId, provider);
  return {
    host: conn.smtpHost,
    port: conn.smtpPort,
...
export async function sendSmtpMail(
  ...
  const conn = await getMailboxConnection(userId, provider);
  return sendViaConnection(conn, input);
}
...
async function sendViaConnection(
  conn: Awaited<ReturnType<typeof getMailboxConnection>>,
  input: SmtpSendInput
): Promise<string> {
